...
main:	
	...
	jal	square				# Jump and link to subroutine
	nop
...
square:	
	sub	$fp, $sp, 16			# make room for $ra, $a0, $s0, $s1 4*4 = 16
	move	$sp, $fp			# stack pointer and frame pointer point to top of stack
	
	sw	$ra, 12($fp)			# push return address
	nop
	sw	$a0, 8($fp)			# push arg
	nop
	...
	bleu	$a0, $t1, returnOne		# if ($a0 = n) < = 1, go to return one
	nop	

	jal	triangle			# Call Triangle(N)
	nop					# Return Value in $v0
	sw	$v0, 4($fp)			# push Triangle(N)	
	nop
	
	lw	$a0, 8($fp)			# Load Arguement back
	nop
	sub	$a0, $a0, 1			# N = N-1
	
	jal	triangle			# Call subroutine witharguement Triangle (N - 1)
	nop
	sw	$v0, 0($fp)			# push Triangle(N-1)
	nop
	lw	$t0, 4($fp)			# $t0 <= load Triangle(N)
	nop

	lw	$t1, 0($fp)			# $t1 <= load Triangle(N-1)
	nop
	addu	$v0, $t0, $t1			# Return value = Triangle(N) + Triangle(N-1)
returnOne:	
	lw	$ra, 12($fp)			# Load Return address to get back to caller
	nop
	lw	$a0, 8($sp)			# Bring back original Arg val
	nop
	add	$sp, $fp, 16			# Pop the stack frame			
