...
void floatnegativecheck(float&, bool&);
void intnegativecheck(int&);
...
int main( )
{
    ...
    while(again == 'y')
    {
        ...
        do
        {
            ...
            if(minvalue < 0)
            {
                cout << "\n\tYou cannot have passengers less than 0. \n\tPlease Enter an acceptable minimum.";
                cout << "\n\n\tHit Any Key to Continue.";
                getch();
            }
...
void floatnegativecheck(float cost&, bool condition&)
{
    if(*cost < 0)
        {
            cout << "\n\tYou have entered a negative value, the computer will automatically turn it into its absolute value.";
            *cost = fabs(*cost);
            cout << "\n\tThe input is now equal to $" << *cost;
            cout << "\n\tHit Any Key to Continue\n";
            getch();
            *condition = true;
        }
}
void intnegativecheck(int people&, bool condition&)
{
    if(*people < 0)
        {
