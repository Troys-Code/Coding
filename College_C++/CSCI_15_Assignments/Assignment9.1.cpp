...
//const is used to make sure the program doesn't change it at any time.
bool compare(const record &last1, const record &last2)
{
    string temp1 = last1.lastName;
    string temp2 = last2.lastName;
    return temp1 < temp2;
}
    ...
    do
    {
        ...
        switch(option)
        {
            ...
            case 2:
            {
                ...
                for(int n = 0; n < width; n++)
                {
                    temporary.push_back(string());
                    temporary[n] = Employee[n].ID;
                }
                it = temporary.begin();
                it = find(temporary.begin(), temporary.end(), temp.ID);

                if(it != temporary.end())
                {
                    int index = distance(temporary.begin(), it);
                    Employee[index].Display(index);
                    status = true;
                    break;
                }
                if(status == false)
            ...
            case 3:
            {
                ...
                char again;
                cout << "\n\n\tDo you want to save your changes? (y or n): ";
                cin >> again;
                again = tolower(again);
                if(again == 'y')
                {
                    //This makes permanent changes to the Data File
                    sort(Employee.begin(), Employee.end()-1, compare);
                    for(int n = 0; n < width; n++)
                    {
                       Employee[n].Display(n);
                    }
